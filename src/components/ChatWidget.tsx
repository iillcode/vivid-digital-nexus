...
const ChatWidget = () => {
  ...
  return <div className="fixed bottom-6 right-6 z-50">
      {/* Chat Toggle Button */}
      {!isOpen && <Button onClick={() => setIsOpen(true)} className="bg-gradient-to-r from-blue-500 via-purple-600 to-blue-500 hover:from-blue-600 hover:via-purple-700 hover:to-blue-600 text-white p-5 rounded-full shadow-2xl hover:shadow-blue-500/25 transition-all duration-300 hover:scale-110 animate-pulse">
          <MessageCircle className="h-7 w-7" />
        </Button>}

      {/* Chat Window */}
      {isOpen && <Card className="w-96 h-[630px] bg-slate-900/95 backdrop-blur-xl border border-blue-500/30 shadow-2xl shadow-blue-500/20 max-w-96 flex flex-col overflow-hidden py-0 px-0 mx-0 my-[20px]">
          <CardHeader className="pb-4 bg-gradient-to-r from-blue-500 via-purple-600 to-blue-500 text-white rounded-t-lg flex-shrink-0 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between relative z-10">
              ...
              <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} className="text-white hover:bg-white/20 h-9 w-9 rounded-full transition-all duration-200 hover:scale-110">
                <X className="h-5 w-5" />
              </Button>
            </div>
          ...
          <CardContent className="p-0 flex-1 flex flex-col min-h-0 bg-gradient-to-b from-slate-900 to-slate-800">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 min-h-0 scrollbar-thin scrollbar-thumb-blue-500/20 scrollbar-track-transparent">
              {messages.map(message => <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-4 rounded-2xl transition-all duration-200 hover:scale-105 ${message.isUser ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg shadow-blue-500/25' : 'bg-slate-700/80 backdrop-blur-sm text-gray-100 border border-slate-600/50'}`}>
            ...
            <div className="p-5 border-t border-slate-600/50 bg-slate-800/95 backdrop-blur-sm flex-shrink-0">
              <div className="flex space-x-3">
                <textarea value={inputText} onChange={e => setInputText(e.target.value)} onKeyPress={handleKeyPress} placeholder="Ask me about TechNova..." className="flex-1 bg-slate-700/80 text-white placeholder-gray-400 border border-slate-600/50 rounded-xl px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent min-h-[50px] max-h-24 backdrop-blur-sm transition-all duration-200" rows={1} disabled={isLoading} />
                <Button onClick={sendMessage} disabled={!inputText.trim() || isLoading} className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white p-3 flex-shrink-0 rounded-xl shadow-lg shadow-blue-500/25 transition-all duration-200 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
